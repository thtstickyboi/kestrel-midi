...
/// Everything the driver hands a backend that `materialise` decides, and no \[2\]
#[derive(Default)]
struct Recorder {
    cmds: Vec<crate::voice::SpawnCmd>,
    glide: Vec<bool>,
...
impl Backend for Recorder {
    fn set_gates(&mut self, _: &[u32], _: &[u32]) -> Result<()> {
        Ok(())
    }
    fn set_channels(&mut self, _: &[u32], _: bool, _: bool, _: bool, _: bool) -> Result<()> {
        Ok(())
    }
    fn set_params_variant(
        &mut self,
        _: u32,
        _: &[crate::bank::RegionParams],
        _: &[crate::bank::ModEnvParams],
    ) -> Result<()> {
        Ok(())
    }
    fn set_glide(&mut self, active: bool) -> Result<()> {
        self.glide.push(active);
        Ok(())
    }
    fn spawn(&mut self, cmds: &[crate::voice::SpawnCmd]) -> Result<()> {
        self.biggest = self.biggest.max(cmds.len());
        self.cmds.extend_from_slice(cmds);
        Ok(())
    }
    fn submit(&mut self) -> Result<()> {
...
/// `Driver::materialise` builds a big block's voices on several threads, each \[3\]
#[test]
fn voices_built_on_any_number_of_threads_are_the_same_bytes() {
    for analytic in [false, true] {
        for delayed in [false, true] {
            for max_voices in [1 << 17, 50_000] {
                let mut cfg = config();
                cfg.max_voices = max_voices;
                cfg.max_block_candidates = 1 << 20;
                if !analytic {
                    cfg.phase = PhaseSettings::default();
                }
                let fixture = Fixture::new(&cfg, delayed);
                // [4]
                let mut m = MidiWriter::new(32767);
                m.tempo_track(500_000);
                let mut events = vec![(0, [0xB0, 65, 127], 3), (0, [0xB0, 5, 30], 3)];
                for i in 0..40_000u64 {
                    let (ch, key) = ((i % 16) as u8, 21 + (i * 7 % 88) as u8);
                    events.push((i % 1000, [0x90 | ch, key, 1 + (i % 127) as u8], 3));
                    events.push((50_000, [0x80 | ch, key, 0], 3));
                }
                m.track(events);
                let midi = fixture.dir.join("burst.mid");
                m.save(&midi).unwrap();
                let p = PhaseBank::prepare(&fixture.bank, &cfg.phase).unwrap();

                let run = |threads| {
                    let cfg = Config { materialise_threads: threads, ..cfg.clone() };
                    let mut driver =
                        Driver::open_prepared(&cfg, fixture.bank.clone(), &midi, p.clone()).unwrap();
                    let mut rec = Recorder::default();
                    let mut block = vec![0.0; cfg.block_samples()];
                    while driver.next_block(&mut rec, &mut block).unwrap() {}
                    rec
                };
                let one = run(1);
                let what = format!("analytic {analytic}, delayed {delayed}, {max_voices} voices");
                assert!(
                    one.biggest >= 2 * crate::driver::MATERIALISE_MIN,
                    "{what}: the biggest block admitted {}, too few to be split",
                    one.biggest
                );
                assert!(one.glide.contains(&true), "{what}: nothing glided");
                for threads in [2, 3, 16] {
                    let many = run(threads);
                    assert!(
                        bytemuck::cast_slice::<_, u8>(&many.cmds) == bytemuck::cast_slice::<_, u8>(&one.cmds),
                        "{what}: {threads} threads built different voices from one"
                    );
                    assert_eq!(many.glide, one.glide, "{what}: {threads} threads");
                }
            }
        }
    }
